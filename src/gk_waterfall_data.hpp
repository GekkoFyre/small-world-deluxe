...
/**
 * @brief GkWaterfallData
 * @author Amine Mzoughi <https://github.com/embeddedmz/QwtWaterfallplot>,
 * Phobos A. D'thorga <phobos.gekko@gekkofyre.io>
 * @tparam T
 */
template <class T>
class GkWaterfallData : public QObject, public QwtMatrixRasterData {
    static_assert(std::is_arithmetic<T>::value, "The waterfall data must be numeric in type!");

public:
    explicit GkWaterfallData(double dXMin, double dXMax, // bounds for x-axis
                             const size_t historyExtent, // define's Y-axis width
                             const size_t layerPoints) : m_data(new T[historyExtent * layerPoints]),
                                                         m_offset(0),
        ...
        setInterval(Qt::XAxis, QwtInterval(dXMin, dXMax, QwtInterval::ExcludeMaximum));
        setInterval(Qt::YAxis, QwtInterval(m_offset, m_maxHistoryLength + m_offset, QwtInterval::ExcludeMaximum));
    }

    ~GkWaterfallData() override {
        delete [] m_data;
   ...
   - BilinearInterpolation\n
    ...
    void getDataRange(double& rangeMin, double& rangeMax) const {
        if (m_currentHistoryLength > 0) {
            auto resultPair = std::minmax_element(m_data + (m_maxHistoryLength - m_currentHistoryLength) * m_layerPoints,
