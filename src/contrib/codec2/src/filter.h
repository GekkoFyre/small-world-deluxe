...
struct quisk_cfFilter {        // Structure to hold the static data for FIR filters
    ...
    _Complex float * cpxCoefs;   // complex filter coefficients
    ...
    _Complex float * cSamples;   // storage for old samples
    _Complex float * ptcSamp;    // next available position in cSamples
    _Complex float * cBuf;       // auxillary buffer for interpolation
} ;

extern int quisk_cfInterpDecim(_Complex float *, int, struct quisk_cfFilter *, int, int);
...
extern void quisk_ccfFilter(_Complex float *, _Complex float *, int, struct quisk_cfFilter *);
