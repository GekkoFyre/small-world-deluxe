    ...
    try {
        ...
        while (stream->isOpen()) {
            ...
            if (!raw_audio_data.empty()) {
                ...
                std::future<std::vector<Spectrograph::RawFFT>> fut_stft = std::async(std::launch::async, std::bind(&SpectroFFTW::stft, spectro_fftw, &conv_audio_data, AUDIO_SIGNAL_LENGTH, hanning_window_size, FFTW_HOP_SIZE, audio_buffer_size, 1));
                while (fut_stft.wait_for(std::chrono::milliseconds(CPP_MULTITHREADING_TIMEOUT)) == std::future_status::timeout) {
                    // Wait and until the data is generated by the STFT code!
                    std::this_thread::sleep_for(std::chrono::duration(std::chrono::milliseconds(CPP_MULTITHREADING_REFRESH_CYCLE)));
                }

                waterfall_fft_data = fut_stft.get();
